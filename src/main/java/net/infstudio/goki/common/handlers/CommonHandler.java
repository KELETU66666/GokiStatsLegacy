...
import net.infstudio.goki.client.gui.GuiInventoryTab;
...
@GameRegistry.ObjectHolder("gokistats_legacy")
public class CommonHandler {
    ...
    public void inventoryInit(GuiScreenEvent.InitGuiEvent.Post event)
    {
        if(event.getGui() instanceof GuiInventory && GokiConfig.inventoryIconEnabled)
            event.getButtonList().add(new GuiInventoryTab(7502, (event.getGui().width/2)-114, (event.getGui().height/2)+GokiConfig.iconYOffset, ""));
